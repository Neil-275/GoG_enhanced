    ...
    def __init__(self, args, mode='train'):
        ...
        if not facts_exists and not train_exists:
            raise FileNotFoundError(
                f"Expected at least one of 'facts.txt' or 'train.txt' in {task_dir}, but neither exists."
            )
        ...
        merged_triples += self.train_triple
        self.all_triple = np.array(merged_triples, dtype=np.int64)
