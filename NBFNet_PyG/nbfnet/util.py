...
try:
    ...
    from NBFNet_PyG.nbfnet import models, datasets
...
def create_working_directory(cfg):
    ...
    # Normalize user-provided paths before we `chdir()` into the experiment folder.
    # Otherwise, relative paths (e.g. "../data_for_LP/raw/family") would be
    # interpreted relative to the new working directory and point to the wrong place.
    if "dataset" in cfg and isinstance(cfg.dataset, (dict, easydict.EasyDict)) and "root" in cfg.dataset:
        cfg.dataset.root =  os.path.join(os.getcwd(),cfg.dataset.root)
    if "checkpoint" in cfg:
        cfg.checkpoint = os.path.join(os.getcwd(),cfg.checkpoint)
    ...
    print("Changed working directory: %s" % working_dir)
    return working_dir
...
def build_dataset(cfg):
    ...
    elif cls == "LocalKnowledgeGraph":

        dataset = datasets.LocalKnowledgeGraph(**cfg.dataset)
    else:
...
def build_model(cfg):
    ...
    if "checkpoint" in cfg:
        # print("Loading checkpoint from %s" % cfg.checkpoint)
        print("abs checkpoint path: %s" % os.path.abspath(cfg.checkpoint))
