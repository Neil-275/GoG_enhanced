...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def propagate(self, edge_index, size=None, **kwargs):
        ...
        # `MessagePassing` private APIs changed across torch_geometric versions.
        # Try to use the fused path when compatible; otherwise fall back to the
        # standard `MessagePassing.propagate` implementation.
        # Newer PyG versions changed the Inspector interface; if so, skip fusion.
        if not hasattr(self.inspector, "distribute"):
            return super(GeneralizedRelationalConv, self).propagate(edge_index, size, **kwargs)
        check_input = getattr(self, "__check_input__", None) or getattr(self, "_check_input", None)
        collect = getattr(self, "__collect__", None) or getattr(self, "_collect", None)
        fused_user_args = getattr(self, "__fused_user_args__", None)
        if fused_user_args is None:
            fused_user_args = getattr(self, "_fused_user_args", None)
        if check_input is None or collect is None or fused_user_args is None:
            return super(GeneralizedRelationalConv, self).propagate(edge_index, size, **kwargs)
        ...
        try:
            ...
            size = check_input(edge_index, size)
            coll_dict = collect(fused_user_args, edge_index, size, kwargs)
        except Exception:
            return super(GeneralizedRelationalConv, self).propagate(edge_index, size, **kwargs)
