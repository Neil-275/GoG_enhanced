...
from GoG_v2.GoG_env_tools import KGEnv
import pandas as pd
...
def is_cvt_like(entity: str) -> bool:
    return str(entity).startswith(("m.", "g."))
...
def merge_finish_candidates(model_candidates, workspace_candidates):
    combined = []
    for item in list(model_candidates or []) + list(workspace_candidates or []):
        item = str(item).strip()
        if not item or item.lower() == "unknown" or is_cvt_like(item):
            continue
        combined.append(item)
    return dedupe_preserve_order(combined)


def split_finish_candidates(candidates):
    real_candidates = []
    cvt_candidates = []
    unknown_seen = False
    for candidate in candidates or []:
        ...
        if candidate.lower() == "unknown":
            unknown_seen = True
            continue
        if is_cvt_like(candidate):
            cvt_candidates.append(candidate)
        else:
            real_candidates.append(candidate)
    return real_candidates, cvt_candidates, unknown_seen
...
def find_answer(process_idx, idxes_to_process, args, datas, env: KGEnv):
    ...
    for n, idx in enumerate(idxes_to_process):
        ...
        try:
            ...
            for _ in range(6):
                known_triples = env.get_known_triples()
                if known_triples:
                    workspace = env.build_reasoning_workspace()
                    env.append_reasoning_records(workspace)
                else:
                    workspace = {
                        "known_triples": [],
                        "path_candidates": [],
                ...
                prompt = base_prompt + env.convert_records_to_str()
                ...
                while not legal:
                    ...
                    try:
                        ...
                        if 'Finish' in thought_action:
                            ...
                            logger.debug(f"other thought: {thought_action}")
                            thought = thought_action.strip().split("\n")[0]
                            action = "Path[raw_thought]"
                            legal = True
                        # n_badcalls += 1
                        # n_calls += 1
                        # thought = thought_action.strip().split("\n")[0]
                ...
                if match:
                    ...
                    real_candidates, cvt_candidates, unknown_seen = split_finish_candidates(prediction)
                    workspace_candidates = env.get_supported_answer_candidates(workspace)
                    merged_candidates = merge_finish_candidates(real_candidates, workspace_candidates)
                    pending_entities = env.get_expansion_entities(workspace)

                    if unknown_seen and not real_candidates:
                        # Finish["unkown"]
                        ...
                        assert env.last_action.lower() in ["search", "path", "rule"]

                        i = len(env.records) + 1

                        if pending_entities:
                            action = "Search[" + " | ".join(pending_entities) + "]"
                        else:
                            action = "Search[ALL]"
                        n_expand += 1
                        if n_expand >= args.max_n_expand:
                            logger.debug("max n_expand, break")
                            prediction = merged_candidates if merged_candidates else answer_question_without_kg(env, base_prompt, args)
                            done = True
                    else:
                        if pending_entities and n_expand < args.max_n_expand:
                            logger.debug("finish requested before all paths were expanded; expanding first")
                            if merged_candidates:
                                prediction = merged_candidates
                            elif real_candidates:
                                prediction = real_candidates
                            elif cvt_candidates:
                                prediction = cvt_candidates
                            else:
                                prediction = prediction
                            action = "Search[" + " | ".join(pending_entities) + "]"
                            n_expand += 1
                            if n_expand >= args.max_n_expand:
                                prediction = merged_candidates if merged_candidates else (real_candidates or cvt_candidates or ["unknown"])
                                done = True
                        else:
                            if merged_candidates:
                                prediction = merged_candidates
                                ...
                                action = "Search[" + " | ".join(cvt_candidates) + "]"
                                n_expand += 1
                                if n_expand >= args.max_n_expand:
                                    prediction = answer_question_without_kg(env, base_prompt, args)
                                    done = True
                ...
                record = {"i": i, "thought": thought, "action": action}
                if match:
                    record["proposed_finish"] = prediction
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_dir", default='GoG_v2/prompts_v3', type=str)
